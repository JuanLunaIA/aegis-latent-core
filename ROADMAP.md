...
**In-place MMR v2 upgrade for an existing chain — the part still open.** `mmr_hash_scheme` (default `auto`) now starts every *new* chain on `aegis-mmr-inclusion-v2`, and the `aegis_rust` accumulator implements v2 as well as v1 (`aegis_rust_v2/src/mmr.rs`), so this is no longer blocked by the accelerator. What remains open is only the reverse case: a chain `auto` opened, or that was pinned, on v1 has no in-place upgrade to v2. A root cannot be recomputed under a new construction without rewriting the history it already committed to, so moving such a chain means a recorded transition (a new chain segment, not a silent rewrite), and nothing implements that transition today. See `CLM-064`.
