...
**Current source line:** `5.0.1` with fourteen synchronized anchors — **published 2026-09-24 on every surface except PyPI `aegis-latent-core`**, read back the same day (`docs/RELEASE_STATUS.md` §1.0a); the previous release is `v5.0.0`, published 2026-09-16 on every surface except PyPI `aegis-latent-core`, whose latest remains `4.1.2` — see `docs/RELEASE_STATUS.md` §1.0
**Most recent published line:** `4.1.2`, read back on 2026-09-04 on every surface
...
|---|---|
| `4.1.x` | Current supported line and most recently published (`v4.1.2` tag, GitHub Release, PyPI core and SDK, npm SDK, GHCR images, read back 2026-09-04). |
...
The current supported line is `4.1.x`, with `4.1.2`, which remains the most recent version published on *every* surface. The checked-out source is `5.0.1` with fourteen synchronized anchors — published 2026-09-24 on every surface except PyPI `aegis-latent-core` (read back the same day, `docs/RELEASE_STATUS.md` §1.0a). The previous release is `5.0.0`: fourteen synchronized anchors, published on 2026-09-16 to the tag, the GitHub Release, PyPI `aegis-latent-sdk`, npm `aegis-latent-sdk` and both GHCR images, but **not** to PyPI `aegis-latent-core`; `cosign verify` and `gh attestation verify` were not run for it. Readback on 2026-09-04 confirmed the signed annotated tag `v4.1.2` targeting `860f14177d94c194e5ae7156017d6fa74264e429`, its GitHub Release with 31 assets, PyPI `aegis-latent-core` `4.1.2`, PyPI `aegis-latent-sdk` `4.1.2`, npm `aegis-latent-sdk` `4.1.2`, and the GHCR gateway and dashboard images. `4.1.2` is the first version at which the gateway itself is on PyPI; the preceding `4.1.1` reached every surface but npm, on a defect in the publish command rather than a policy gate. The previous published line is `4.0.x`: the signed annotated tag `v4.0.2` targets `a6eb58dcc03f8b638c8f3e35f0300f5443a926ca`, and its GitHub Release and GHCR images were read back on 2026-09-02. The previous public GitHub baseline is lightweight tag `v4.0.1` targeting `6469904380218584ae0b5221334bc9a46500f5ba`; its tag workflows failed, while PyPI/npm were separately observed at `4.0.0` without attributed provenance. Release support must be evaluated from published artifacts, while source behavior must be evaluated from the named commit, tests, and deployment prerequisites.

## Distribution channels and what each one enforces

`pip install aegis-latent-core` installs one package carrying both deployment
shapes: the `aegis` / `aegis-server` console scripts that run the gateway as a
separate process, and `aegis.wrap()` that runs the same controls inside the
calling process. `pip install aegis-latent-sdk` and `npm install
aegis-latent-sdk` install proof verifiers only — they enforce nothing and hold
no key. The published core wheel is `py3-none-any`, so the full feature set runs
without the optional `aegis_rust` extension.

**The two shapes are not equivalent security boundaries, and the difference is
the point.** The gateway is a separate process the application cannot bypass:
an application that is itself untrusted still cannot reach the provider without
passing through admission, redaction and the ledger. The embedded engine
governs calls made through the client object it wrapped, and nothing else. Code
in the same process can hold a second unwrapped client, call the provider
directly over its own socket, read or edit the WAL, or read plaintext out of
process memory before redaction ever runs.

Embedded mode is therefore an evidence and policy layer for **cooperative**
application code — it makes the governed path the easy path and produces
records that verify identically to the gateway's. It is not a sandbox, not a
containment boundary against its own process, and not a control you can cite
against an insider or a compromised dependency inside that process. Where the
application is the thing being constrained, deploy the gateway.
