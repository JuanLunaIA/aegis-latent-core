...
| 2026-09-24 | [`v5_0_1_release_readback_2026-09-24.md`](v5_0_1_release_readback_2026-09-24.md) | **Current publication record for `v5.0.1`:** tag signature, Release and `SHA256SUMS` sweep, registries, image signatures and build-provenance attestations, each read back and verified against the exact workflow identity. | Supersedes nothing above for its own scope; `docs/RELEASE_STATUS.md` §1.0a summarizes it. | `v5_0_1_release_readback_2026-09-24.md.sha256` |
...
| [`backpressure_group_commit_remeasurement_2026-09-16.md`](backpressure_group_commit_remeasurement_2026-09-16.md) with [`execution_2026-09-16/`](execution_2026-09-16/) | 2026-09-16 re-execution of `tools/benchmarks/run_backpressure_stall.py` at commit `88e01f0`, three runs at the same parameters as the 2026-08-20 report | **Supersedes `execution_2026-08-20/backpressure_stall_report.json` for current-state citation only.** The older run is at `20fa011`, before the coalesced group-commit engine (`CLM-082`) entered the tree, and remains a correct observation of that tree. Different hosts, so the millisecond delta is not a controlled speedup measurement; the `fsync`-call count (2,501 → 200 for 2,500 records) is the host-independent part. Injected `fsync` seam, not real storage; offered load is not accepted capacity; the latency is queueing under deliberate oversubscription, not per-request overhead. |
