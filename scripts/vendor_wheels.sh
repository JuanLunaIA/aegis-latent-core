...
#   docker build --network=none -f deploy/docker/Dockerfile.airgap -t aegis-latent-core:4.3.0-airgap .
...
pip download \
    ...
    --only-binary=:all: \
    "aegis-latent-core[storage-sqlite]==4.3.0" 2>/dev/null || \
pip download \
    --dest "${WHEELS_ABS}" \
    ".[storage-sqlite]" \
...
if command -v docker &>/dev/null && docker info &>/dev/null 2>&1; then
    ...
    if [[ -n "${DIGEST}" ]]; then
        ...
        echo "[vendor_wheels]     -t aegis-latent-core:4.3.0-airgap ."
    fi
else
