...
"""integration_test_mock.py — comprehensive local verification harness for Aegis v4.1.0.
...
def _run_sweep(base_url: str, api_key: str, timeout: float = 5.0) -> None:
    ...
    print(f"\n{'=' * 72}")
    print(f"  Aegis v4.1.0 Integration Sweep — {len(PAYLOAD_MATRIX)} payloads")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Aegis v4.1.0 integration sweep — drives PAYLOAD_MATRIX against a running proxy"
    )
    parser.add_argument(
        "--sweep",
        action="store_true",
