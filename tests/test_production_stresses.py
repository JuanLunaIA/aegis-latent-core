...
class TestProductionStresses(unittest.TestCase):
    ...
    def test_NEW_04_Deque_Ceiling(self):
        ...
        limit = 100
        ledger = CryptographicAuditLedger(persistence_path=self.path, max_memory_nodes=limit)
    ...
    def test_NEW_07_FD_Leak(self):
        ...
        for _ in range(10):
            with CryptographicAuditLedger(persistence_path=self.path) as ledger:
    ...
    def test_NEW_03_Concurrent_Latency(self):
        ...
        print(
            f"\n[STRESS] Avg commit latency under contention: "
            f"{avg_lat * 1000:.3f}ms"
        )
        ledger.close()
