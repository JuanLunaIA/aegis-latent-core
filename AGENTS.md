...
- The checked-out source baseline is **5.0.0** with fourteen synchronized anchors, and **nothing is published for it** — no tag, no GitHub Release, no PyPI or npm artifact, no OCI image. Source metadata never establishes external lifecycle state on its own: the tag, GitHub Release, PyPI and npm artifacts, OCI digest, signature and attestation are separate observables, and a version in the tree is unpublished until each one is read back. `5.0.0` is exactly that case, so do not describe it as released, available, or installable. **There is no `4.2.0` or `4.4.0`** at any surface; both numbers were skipped deliberately, and their absence is not a withdrawn release. The `4.3.0` → `5.0.0` jump reflects a breaking change to the public JSON API (`CLM-090`: `signature_assurance` replaces `legal_admissibility` on `/audit/health` and `/audit/integrity`), not a version-numbering oversight.
