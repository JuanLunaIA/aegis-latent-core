...
- The checked-out source baseline is **4.1.2** with fourteen synchronized anchors, and it **is published** — see the next bullet for the readback. Source metadata never establishes external lifecycle state on its own: the tag, GitHub Release, PyPI and npm artifacts, OCI digest, signature and attestation are separate observables, and a future version in the tree is unpublished until each one is read back.
