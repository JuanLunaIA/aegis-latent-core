...
- The checked-out source baseline/release target is **4.1.2** with fourteen synchronized anchors, and **nothing is published for it**. Source metadata does not establish external lifecycle state; verify the `v4.1.2` tag, GitHub Release, PyPI and npm artifacts, OCI digest, signature, and attestation through independent readback.
- The most recent published release is `v4.1.1`. Readback on 2026-09-03 established it as published: signed annotated tag at `5a137c86ecd914842493babb7e863033498f68c9`, GitHub Release with 31 assets checking against `SHA256SUMS`, PyPI `aegis-latent-sdk` `4.1.1`, and GHCR gateway and dashboard images with cosign signature objects present. **npm still carries `4.0.0`** — state the two SDK registries separately, never as "the registries". `cosign verify` and `gh attestation verify` were not run.
