...
- The checked-out source baseline is **5.0.1** with fourteen synchronized anchors, and it is the most recent published release: **published on 2026-09-24 to every surface except PyPI `aegis-latent-core`**. Readback established: signed annotated tag `b590a7f40df939b9efa361f3960bfa8b532296c9` at commit `46db6c0c65582bbb46137fd14182a976274b34e8` (`gitsign verify-tag` passed against the `create_release_tag.yml` identity), a GitHub Release with 31 uploaded assets and a full `SHA256SUMS` sweep (15 of 15 verified), PyPI `aegis-latent-sdk` `5.0.1`, npm `aegis-latent-sdk` `5.0.1`, and GHCR gateway `sha256:2d22023e…` and dashboard `sha256:a05b41c5…`. `cosign verify` passed for both images, and the build-provenance attestations of both images and all 15 release files verified, each against the exact publishing workflow identity; `gh attestation verify` itself was not run. **The gateway distribution `aegis-latent-core` is NOT on PyPI at `5.0.1`** — its latest there is still `4.1.2`. The previous release, `5.0.0`, was published on 2026-09-16 to every surface except PyPI `aegis-latent-core`. Readback established: signed annotated tag `c34d41280ec4a5acb38aa74220eac8f20a9d0aab` at commit `b2e4335409377442e9dde70ea579c3df08a0c1be`, a GitHub Release with 31 uploaded assets, PyPI `aegis-latent-sdk` `5.0.0`, npm `aegis-latent-sdk` `5.0.0`, and GHCR gateway `sha256:81d106c9…` and dashboard `sha256:ec73cba5…` with cosign signature objects present. `cosign verify` and `gh attestation verify` were **not** run, and the `SHA256SUMS` sweep was not run for `5.0.0`. **The gateway distribution `aegis-latent-core` is NOT on PyPI at `5.0.0`** — its latest there is still `4.1.2`, so `pip install aegis-latent-core` gets `4.1.2`; no workflow publishes that distribution, so this will not resolve on its own. Source metadata never establishes external lifecycle state on its own: the tag, GitHub Release, PyPI and npm artifacts, OCI digest, signature and attestation are separate observables, and each must be read back before it is claimed. **There is no `4.2.0` or `4.4.0`** at any surface; both numbers were skipped deliberately, and their absence is not a withdrawn release. The `4.3.0` → `5.0.0` jump reflects a breaking change to the public JSON API (`CLM-090`: `signature_assurance` replaces `legal_admissibility` on `/audit/health` and `/audit/integrity`), not a version-numbering oversight.
- The most recent published release is `v4.1.2`. Readback on 2026-09-04 established it as published: signed annotated tag at `860f14177d94c194e5ae7156017d6fa74264e429`, GitHub Release with 31 uploaded assets, PyPI `aegis-latent-core` `4.1.2`, PyPI `aegis-latent-sdk` `4.1.2`, npm `aegis-latent-sdk` `4.1.2`, and GHCR gateway and dashboard images with cosign signature objects present. `cosign verify` and `gh attestation verify` were not run, and the full `SHA256SUMS` sweep was not run for `4.1.2`. **`4.1.2` is the first version at which the gateway is on PyPI** — do not write that the gateway ships from source only or that the registries carry SDKs only; both were true at `4.1.1` and are false now. The PyPI `aegis-latent-core` artifacts are byte-different from the release assets of the same name (identical content, different build host), so `SHA256SUMS` does not cover the PyPI gateway downloads.
