    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        try:
            ...
            if not lsm.verify_confinement():
                if not lsm.is_sandbox:
                    raise RuntimeError("CRITICAL: LSM confinement verification failed.")
                logger.warning("LSM confinement not detected (degraded mode - sandbox detected)")
        except Exception as exc:
            if lsm is not None and not lsm.is_sandbox:
                raise RuntimeError(f"CRITICAL: LSM initialization failed: {exc}") from exc
