...
| Streaming responses are fully buffered under configured per-response byte and total-duration limits and are not emitted before the evidence gate. | `IMPLEMENTED` | `aegis/config.py`, buffered-stream handling in `aegis/proxy/app.py`, and `tests/test_app_coverage_extended.py` byte/deadline tests | The limits bound one request. Aggregate memory remains proportional to concurrent admitted streams and deployment-level concurrency controls. |
