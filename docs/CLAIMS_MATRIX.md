...
| Streaming responses are fully buffered and are not emitted before the evidence gate. | `IMPLEMENTED` | `aegis/proxy/app.py:1089-1137` and proxy streaming tests | No accumulated upstream streaming-response byte bound is implemented; memory-exhaustion resistance for large responses remains unproved. |
