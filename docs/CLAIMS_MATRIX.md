...
| Governed successful and terminal error responses expose durable evidence status. | `IMPLEMENTED` | Proxy tests, enterprise durable-evidence tests, and v3.0.1 release evidence | Falsified by any accepted governed response without a durable evidence record in the declared test scope. |
...
| Aegis implements controls that can support evidence, retention, and policy workflows aligned with selected frameworks. | `LEGAL-REVIEW-REQUIRED` | `docs/compliance/COMPLIANCE_MAPPING.md` | Not a SOC 2 opinion, HIPAA determination, FedRAMP authorization, EU AI Act conformity assessment, or legal advice. |
| Aegis provides constant-time ML-DSA signing or verification. | `ROADMAP` | `docs/security/PQC_CONSTANT_TIME.md`, `pqc_timing_report_v2.json` | The 1M-sample candidate met non-detection for `sign` (`p=0.8521504207157158`) but failed the declared `verify` threshold (`p=0.0`); no constant-time claim is approved. |
...
The original v3.0.1 scan contained 791 claim-bearing lines and 79 drift candidates. The generated v3.1.0 ledger contains 304 claim records and 27 drift candidates in `claims_ledger_v3_1_0.json`; it is retained as a release artifact outside the source tree. The scan is an inventory aid; human review remains required for context, reachability, legal interpretation, and source quality.
