...
The in-tree candidate run (2026-08-20, `20fa011`, before group commit) offered 2,500 requests at 10,000 RPS offered with a 2 ms injected `fsync` delay and 64 workers. It recorded 2,500 durable commits, zero failures, zero missing IDs, zero duplicate IDs, and valid chain integrity. Observed total runtime was 6.630050110979937 s, with commit latency p50 `167.29020897764713 ms`, p95 `504.7037289477885 ms`, p99 `836.3514210795984 ms`, and max `2290.622486965731 ms` (`evidence/execution_2026-08-20/backpressure_stall_report.json`, 2,501 `fsync` calls). The current source baseline (`88e01f0`) reports the same 2,500-request workload draining in about 1.6 s at p99 51.87 ms (`evidence/execution_2026-09-16/`). The previously published `v3.1.0` run — 10,000 requests, p99 `1189.8909930023365 ms` — is retracted (`UC-018`): its raw JSON was never committed, so nothing in this tree produces those figures. This proves the no-silent-drop behavior within that local harness boundary; it also demonstrates that the offered load caused severe queueing and does not establish accepted capacity or a production SLO for v3.1.0 or v4.
