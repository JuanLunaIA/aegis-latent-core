...
**Cost.** The key vault becomes the only mutable component in an append-only design, and therefore a new single point of failure: lose it and every subject's plaintext is gone at once; compromise it and every erasure through it is undone. Its backup policy is in direct tension with the erasure it exists to perform. Erasure is bounded to *unrecoverable to a holder of the ciphertext* — it says nothing about key material on the physical medium, where allocator copies, the SQLite journal, page cache, swap, snapshots and SSD wear-levelling are all outside a CPython process's reach — and the node keeps its request and response digests — keyed to the subject under the current `v2` scheme, so a guess is no longer confirmable once the key is destroyed, but plain `SHA-256` for a legacy `v1`-sealed node or any node written with shredding off (`CLM-098`). The option is **off by default**, and sealing is decided per commit rather than per chain: `AEGIS_ENABLE_CRYPTOGRAPHIC_SHREDDING` can be turned on for a chain that already holds records, sealing everything committed from that point on while leaving earlier leaves as written, and the mixed chain still verifies. (This paragraph previously said adoption required starting a new chain, by analogy to `AD-12`'s hash-scheme rule; that was wrong and is corrected 2026-09-16 — `AD-12` itself is unchanged. See `CLM-068`.) It still ships off, for the reason the costs above give: paying the WAL-growth and vault-custody costs should be a deliberate act, not the consequence of an upgrade. The asymmetry that remains is that nothing seals leaves already committed as plain digests, so enabling it does not make history shreddable. And it settles a structural conflict, not a legal one: **no regulatory conclusion follows.** Governed by `CLM-068`; boundaries in [DOC-02 §6.2](../institutional/DOC-02_CRYPTOGRAPHIC_FORENSIC_BLUEPRINT.md) and [DOC-05 §5.8.1](../institutional/DOC-05_REGULATORY_DOSSIER.md).
...
## AD-16 — Decline a ledger-level multi-pod total order, and keep the single-writer contract

**Decision.** The gateway ledger stays **single-writer per chain**. For the ledger, the repository declines all three mechanisms that would give concurrent writers one global order — a centralized writer, an external storage provider's atomic compare-and-append path, and a consensus round. Cross-pod ordering stays published open work rather than an implied capability, and the supported answer to "we need multi-pod evidence" remains the one already documented: one writer per chain, with per-chain bundles that a reader reconciles explicitly.

**Rejected.** (a) A centralized writer for the gateway ledger. (b) Running the gateway ledger on an external provider's atomic compare-and-append append. (c) A consensus round (Raft) over ledger appends.

**Why.** (b) needs saying out loud because it stopped being hypothetical: `write_node_atomic` (`aegis_server/storage/base.py:245`) appends only if the chain tip is still `expected_prev_hash`, and raises `ConcurrentChainMutationError` instead of forking — SQLite holds its write lock across the check and the insert (`aegis_server/storage/sqlite_provider.py:378`), PostgreSQL and DynamoDB implement the same contract (`:296`, `:237`), it is already the enterprise server's append path (`aegis_server/main.py:593`), and it is covered by tests (`tests/storage/test_chain_fork_prevention.py`, `tests/test_postgres_concurrent_append_race.py`, `tests/test_dynamodb_concurrent_append_race.py`; introduced in #156). The gateway still does not call it, and the reason is structural rather than unfinished work: the gateway's ledger is a local JSONL WAL whose custody, signing and evidence semantics are the product's core claim, while `aegis_server` is a separate surface with a separate contract and a different deployment shape. Adopting it would move custody of the authoritative record to an external service and make the evidence boundary depend on that service's atomicity — that is a different product, not a fix in this one. (a) and (c) were already declined for the accumulator in `AD-03` and `AD-13` on the same grounds: capacity, availability, failover, recovery and custody are all real work that has not been done, and shipping a sequence nobody can operate is worse than publishing a boundary. The single-writer contract is published, not merely implemented: `UC-005` and [DOC-01 §8.4](../institutional/DOC-01_ENTERPRISE_ARCHITECTURE.md) state the limitation, and `REG-A01` closed it as the documented single-node contract.

**Cost.** Multi-pod deployments must shard by chain and reconcile per-chain evidence themselves; there is no merged root over the *ledger* (the accumulator's root, `AD-13`, is not the same object), and a reader who wants one global sequence does not get one. The declined path also stays implemented and maintained: `write_node_atomic` is covered by tests for a deployment shape the gateway does not offer, and a future reader may reasonably ask why it is not wired — this entry is the answer. The decline is revisitable: if the custody question is ever answered — who holds the authoritative record, and what a provider outage costs — (b) is the shortest path, and this entry should be revised rather than quietly ignored. Governed by `CLM-012`; boundaries in `UC-005`, [DOC-01 §8.4](../institutional/DOC-01_ENTERPRISE_ARCHITECTURE.md) and [BOUNDARIES](../BOUNDARIES.md).
