...
This document explains the Aegis request lifecycle, evidence boundary, state machine, trust boundaries, topology choices, and failure semantics in the `5.0.0` source baseline. It is for platform engineers, security reviewers, developers, and buyer technical evaluators. It does not establish package publication, deployment acceptance, global ordering, multi-region availability, a production SLO, or regulatory certification.
...
**Historical evidence baseline:** retained `v3.1.0` artifacts and measurements remain historical; `v5.0.0` source behavior is not attributed to that distribution
...
## Components whose presence in the tree is not the same as their being on the request path

A tested component and a running control are different claims, and the distinction should be visible from the architecture document rather than only from the claims matrix. Of the three components this section used to list as unwired, two now reach the request path and one still does not — and the two that do reach it arrive under conditions worth stating precisely.

| Component | What it is | What actually reaches the request path |
|---|---|---|
| `CausalMmr` (`aegis_rust_v2/src/crdt_mmr.rs`) | A join-semilattice over domain-separated MMRs; `join` is idempotent, commutative and associative, so replicas exchanging leaf sets converge on one root whatever order they merge in | **Nothing.** There is no membership protocol or persistence, and a merged root commits to a set of leaves rather than re-linking per-replica `prev_hash` chains. An anti-entropy gossip transport over mutual TLS does now run — started from the gateway's own lifespan, behind an opt-in that is off by default — but it reconciles the accumulator off the request path, not the ledger (`CLM-080`). Cross-replica ordering stays open work. `CLM-066`, [DOC-01 §8.8](../institutional/DOC-01_ENTERPRISE_ARCHITECTURE.md) |
| `GrammarFrontierAutomaton` (`aegis/core/streaming_safety_engine.py`) | A streaming redactor that runs to fixpoint before computing a withheld frontier that is a real bound, because every pattern quantifier is bounded | **Wired, on by default.** `aegis/core/stream_redactor.py` composes it *after* `StreamingDeidentifier`, so the seventeen Safe Harbor detectors and the automaton's four rules both run; it did not replace the de-identifier, and the rolling digest still covers what the composite emits. `AEGIS_STREAMING_ENGINE=deidentifier` restores the pre-`4.3.0` output exactly. With both PHI and PCI families disabled the stage does not run at all, so a deployment with redaction off keeps byte-for-byte pass-through. `CLM-067`, [DOC-03 §5.4](../institutional/DOC-03_THREAT_MODEL.md) |
| `CryptoShredder` (`aegis/core/crypto_shredder.py`) | Per-subject AES-256-GCM envelope encryption; the ledger commits the ciphertext, so destroying a key leaves the root, peaks and prior proofs bit-for-bit unchanged | **Wired, off by default.** With `AEGIS_ENABLE_CRYPTOGRAPHIC_SHREDDING=true` the ledger seals each leaf and commits `SHA-256(0x00 \|\| nonce \|\| ciphertext)`; unset, it commits payload digests exactly as before. Sealing is a **per-commit decision**, so the flag can be enabled on a chain that already holds records: later leaves are sealed, earlier ones stay as written, and the mixed chain still verifies (`AuditNode.shredding_version` distinguishes them). Nothing seals already-committed leaves retroactively. `CLM-068`, [DOC-02 §6.2](../institutional/DOC-02_CRYPTOGRAPHIC_FORENSIC_BLUEPRINT.md), [DOC-05 §5.8.1](../institutional/DOC-05_REGULATORY_DOSSIER.md) |

The `aegis-mmr-inclusion-v2` scheme is selectable on the ledger through `AEGIS_MMR_HASH_SCHEME`, and its default `auto` starts a **new** chain on v2 while reopening an **existing** chain under whatever scheme its WAL recorded. So a fresh deployment writes v2 roots and a deployment already in the field keeps writing v1 ones; there is no in-place upgrade, because a root cannot be recomputed under a different construction without rewriting the history it commits to (`CLM-064`, [AD-12](DECISIONS.md)).
