...
| **Registry lag** — the gateway distribution `aegis-latent-core` is still `4.1.2` on PyPI while the source and both SDKs are `5.0.0` | Medium | Install the SDKs from the registry at `5.0.0`; a PyPI gateway install is `4.1.2`, so install from source for `5.0.0` |
