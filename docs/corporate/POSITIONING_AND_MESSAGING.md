...
**Be honest early about:** `pending-terminal` semantics, one worker per WAL path, and the SDK registry lag — a developer who installs `4.0.0` from PyPI and reads `4.0.2` documentation will hit a mismatch and lose trust.
...
| Published on PyPI at 4.0.2 | SDKs are at 4.0.0; source is at 4.0.2 |
