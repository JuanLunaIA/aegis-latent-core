...
- [x] **AUD-12 [P2] — Rust forwarder buffers upstream responses with no cap**
  ...
  - **Closed 2026-09-21 (`REG-D16`):** the body is read under a cap in `forwarder.rs`; the declared length is a cheap refusal and the counted chunks are the authoritative bound. Before: `resp.bytes().await` collected the whole body and `HttpResponse.content` copied it again, with no in-crate limit to breach (the stale extension's signature had no cap parameter — observed on this host). After: `cargo test --locked` 81 lib tests passed (incl. the three new `forwarder::` cases) and `tests/test_rust_forwarder_response_cap.py` 5 passed against the freshly built cdylib — a 32,768-byte body against a 4,096-byte cap raises `RuntimeError` naming the limit, a 2,048-byte body returns intact, the no-argument default admits 32,768 bytes, and `max_response_bytes=0` is refused at construction. `clippy --locked --all-targets -- -D warnings` exit 0. The Python non-streaming httpx path (§AUD-23) keeps its own bound.
- [x] **AUD-13 [P2] — RustWaf documents NFKC normalisation that does not exist**
  ...
  - **Estimated effort:** S-M (1-2 days)
  - **Closed 2026-09-21 (`REG-D17`):** documentation branch taken — the crate performs no Unicode normalisation, the claim said otherwise, and the gateway's Layer 1 already denies what the pre-filter misses, so the false API statement is corrected and the false negative published (`UC-063`) rather than adding a `unicode-normalization` dependency and a second normalisation authority that would drift from `_normalize_text`. Two crate tests pin both halves: the variant spellings pass the Rust scanner (failing on purpose if NFKC is ever implemented), and the NFKC form is what the crate blocks. Reproduced the four reported rows on this host and measured the end-to-end denial with the pre-filter active. `cargo test --locked` waf 7 passed.
- [x] **AUD-14 [P2] — Inert configuration controls presented as enforceable (CAC/PIV, PHI at-rest key, LDAP family)**
  ...
  - **Closed 2026-09-21 (`REG-D18`):** per control — `phi_master_key` deleted (nothing read it, and the WAL holds digests rather than payload bytes, so the promised encryption had no bytes to act on; `UC-067`), the ten unread `ldap_*` fields and `cac_piv_required` labelled *not wired* with their intended semantics kept as the specification (`UC-064`, `UC-065`), the two encryptor modules' docstrings corrected so they no longer tell operators to set a variable the gateway does not read, and the ticket's third element built: `tests/test_config_surface_inert_fields.py`, which fails when a settings field has no production reader and when an allowlisted entry acquires one (both directions verified with live negative controls). The gate's first run surfaced three more unread fields (`rate_limit_window`, `webhook_url` in both settings classes) → `AUD-35`; their operator-facing copies (presets, Helm, compose, the deployment-profiles table) are labelled in the same commit.
...
- [x] **AUD-18 [P2] — Gate-scope drift: the Makefile formatter gate is broader than CI's and fails at HEAD**
  ...
  - **Closed 2026-09-21 (`REG-D22`):** canonical scope chosen as the whole tree over a fixed path list — `ruff check .` / `ruff format --check .` in both the Makefile and CI, with markdown excluded from formatting in `[tool.ruff.format]` so the 9 doc/README files whose fences ruff 0.16 formats are not rewritten by a formatter (the doc gates own that content). The two named scripts are reformatted and now inside both gates; the same drift in `make security` (bandit `-ll` over `aegis/` versus CI's `-lll` over both trees) and `make type` (`--ignore-missing-imports` versus CI's `--strict aegis`) is fixed too. `tests/test_gate_scope_parity.py` fails if either file's ruff/bandit invocation stops matching the other's, with live negative controls in both directions. Measured: `make lint` 1 → 0, "11 would be reformatted" → 0 of 578; `make type` and `make security` both exit 0.
- [x] **AUD-19 [P2] — Documentation currency & provenance sweep (stale baselines and counts)**
  ...
  - **Closed 2026-09-21** — this commit: `llms.txt`, the twelve-file `.aegis_ai_context/` corpus (including the generator constants that feed the generated manifest), `docs/REPOSITORY_MAP.md` and `docs/README.md` all name the checked-out baseline `5.0.0`; `docs/INDEX.md` covers 108 of 109 tracked documents (the exception is itself); `INTEGRITY_SEAL.md` carries a currency note recording that its base commit is absent from this repository's history and listing the data-current values; `README.md` carries a 2026-09-21 row. The force multiplier shipped: `tests/test_documentation_currency.py` fails when any of the twelve artifacts names a baseline other than `pyproject.toml`'s anchor, when a tracked document is unreachable from the index, or when `4.1.2` appears as a current claim rather than a published/historical one — each direction proved with a live negative control. Evidence: `evidence/registry/reg-d23_fixed.txt`.
- [x] **AUD-20 [P2] — MiFID II / MAR: modules unwired, citations partly wrong, registers silent**
...
- **Closed 2026-09-21** — this commit: the register silence is closed. `CLM-103` and `CLM-104` added under a new *Regulatory-input claims (built, not wired)* section with explicit not-wired boundaries, the correct instruments (MAR (Reg. (EU) No 596/2014) Art. 12(1)(a)(ii) for spoofing; MiFID II Art. 16(6)/25(1) five-year floor, seven only at a competent authority's request), and the hash-only retention limit; a control-register range `CLM-103`–`CLM-104` forbidding "MiFID II compliant"/"MAR compliant"/"SMCR compliant" with review date and owner; `UC-056` confirmed present and now pinned by a test; the claims matrix's stale `Status:` line corrected and its source-baseline line brought under the `REG-D23` currency gate; the wiring decision opened as **`AUD-36`** rather than made inside a documentation row; the SMCR/counsel question added to the human pack as **`REG-H10`** (wording review only — `AUD-36` does not wait on it). Gate: `tests/test_regulatory_input_claims.py` (7 tests) with three live negative controls (boundary -> wired; citation -> MiFID II Art. 12; module removed from the allowlist). Evidence: `evidence/registry/reg-d24_fixed.txt`.
...
- [ ] **AUD-35 [P2] — Wire or remove the remaining inert settings found by the config-surface gate**
  ...
  - **Proposed solution:** decide per control — either wire it (source `siem_url` from `webhook_url` when empty, feed `rate_limit_window` into the limiter, construct `LDAPAuthenticator`, construct `CACPIVAuth`) with a test per wired path, or delete the knob and its operator-facing copies. The gate (`tests/test_config_surface_inert_fields.py`) enforces that whichever is chosen, the section and the allowlist agree.
  - **Estimated effort:** M (the LDAP and CAC/PIV wire-ups are the larger legs; the webhook alias and the window are S).

- [ ] **AUD-36 [P2] — Wire or retire the two MiFID II / MAR input modules**
  - **Affected files:** `aegis/core/market_abuse_detector.py`; `aegis/core/mifid_record_keeper.py`; `scripts/import_reachability_allowlist.txt:64-65`; `docs/CLAIMS_MATRIX.md` (`CLM-103`, `CLM-104`); `tests/test_market_abuse_detector.py`; `tests/test_mifid_record_keeper.py`
  - **Root cause:** both modules are implemented and tested, and both are imported by nothing the gateway can reach — the repository's own reachability tool lists them as allowlist entries whose header reads "a worklist, not a verdict". AUD-20 closed the *register* silence around them (claim rows, boundary, control register, unsupported claim); it deliberately did not decide their future, because wire-or-retire is a product decision with a configuration surface, a routing choice and a claims rewrite behind it.
  - **Proposed solution:** decide per module. Wire: a governed admission-path hook for the detector with an explicit configuration flag, a documented verdict destination, and rewritten `CLM-103`/`CLM-104` rows with measured evidence; or retire: delete the module, its tests and its allowlist entry, and mark the corresponding claim row withdrawn. Either branch removes the allowlist entry, which is the point — an allowlisted compliance module is a liability in every audit it appears in.
  - **Estimated effort:** M for wiring either module (configuration + routing + claims evidence); S for retiring both.
