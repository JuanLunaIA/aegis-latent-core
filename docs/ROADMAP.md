...
- External lifecycle claims for `v4.0.2` are bounded by independent readback: the signed tag, GitHub Release assets, GHCR manifests, GitHub attestations, and OCI signatures passed; PyPI/npm remain at the observed `4.0.0`, so SDK registry publication is not claimed. Modules A–D still require deployment acceptance before being described as production capabilities.
