...
**Status:** checked-out source baseline `v4.3.0` has 14 synchronized anchors and is unpublished; source metadata does not establish external lifecycle or target-acceptance state
**Last verified:** 2026-09-08 UTC (register reviewed against the `4.3.0` source tree; external readback values are unchanged from their recorded dates)
...
**Source baseline:** fourteen synchronized `4.3.0` anchors, unpublished; immutable parent comparison `fdace8844568eb788216740b2cb5daf187d99d3b` has fourteen `4.0.0` anchors
...
The current **source baseline** is `4.3.0` with fourteen synchronized anchors, and nothing is published for it. The most recent published release is `4.1.2`. Readback on 2026-09-04 confirmed the signed annotated tag `v4.1.2` at `860f14177d94c194e5ae7156017d6fa74264e429`, its GitHub Release with 31 assets, PyPI `aegis-latent-core` `4.1.2`, PyPI `aegis-latent-sdk` `4.1.2`, npm `aegis-latent-sdk` `4.1.2`, and the GHCR gateway and dashboard images. Its immutable parent/source comparison `fdace8844568eb788216740b2cb5daf187d99d3b` has fourteen `4.0.0` anchors. The previous published GitHub baseline is signed annotated tag `v4.0.2` at `a6eb58dcc03f8b638c8f3e35f0300f5443a926ca`; before it, lightweight tag `v4.0.1` targeted `6469904380218584ae0b5221334bc9a46500f5ba` with failed tag workflows, and observed PyPI/npm `4.0.0` objects have no attributed provenance from those runs. Source metadata never establishes an external tag, release, registry artifact, deployed service, SLO, compliance result, or acceptance; every claim above rests on the readback, not on the version in the tree.
...
- [x] **Module B addendum — `aegis-mmr-inclusion-v2` domain separation.** v1 tags neither hash input, so a leaf payload equal to the concatenation of two child digests hashes to exactly the interior node over them — the RFC 6962 §2.1 type-confusion weakness, reachable through `verify_portable_inclusion`, which accepts caller-supplied leaf bytes. v2 prefixes each input with a domain tag and consumes raw 32-byte digests. Locators: `aegis/core/mmr.py` (`v2_leaf_hash`, `v2_node_hash`, `v2_bagged_root`), `tests/test_mmr_domain_separation.py`, `sdk/typescript/tests/mmrV2.test.ts`, `CLM-064`. **v1 remains the default and v2 is not wired into the ledger** — see the open item below.
- [ ] Wire `aegis-mmr-inclusion-v2` into `CryptographicAuditLedger`. Two prerequisites are unmet and neither is a code detail: the `aegis_rust` accumulator implements v1 only, so an accelerated deployment running Python v2 would disagree on every root; and an existing chain cannot change scheme without a recorded transition, because a root cannot be recomputed under a new construction without rewriting the history it already committed to. Until both are addressed, v2 serves callers building their own accumulator and verifiers checking v2 proofs.
...
- [ ] Provide cross-replica global ordering and multi-region failover/recovery evidence; current implementation does not establish multi-region HA. `CausalMmr` supplies the accumulator but not the system: there is no gossip transport, membership, persistence, or replica authentication, and the ledger still writes v1 roots, so each replica remains an independent chain (`CLM-012`). Convergence is also not agreement about truth — `join` reconciles replicas that disagree about ordering, not replicas that lie, so Byzantine resistance is a separate and unaddressed problem.
...
Nothing is published for the current `4.3.0` source baseline. Deployment acceptance remains open until the affected source and tests, target-environment controls, architecture smoke/rollback evidence, SBOM/dependency review, and qualified residual-risk review are complete.
