...
- [~] Add `osv-scanner` over the project lockfiles. **Done:** committed `aegis_rust_v2/Cargo.lock` (it was git-ignored; `aegis_rust` is a `cdylib` extension/application artifact, so a pinned lockfile is correct for reproducible `--locked` builds and scanning — verified consistent with `Cargo.toml` via `cargo metadata --locked`). An `osv-scanner` job (`google/osv-scanner-action` v2, report-only) in `security.yml` now scans both `requirements.txt` (Python) and `Cargo.lock` (Rust) against the OSV database and uploads SARIF to the Security tab. **Remaining:** generate a `requirements.lock` with hashes (`--require-hashes`) for reproducible, audited installs.
