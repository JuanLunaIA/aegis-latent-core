...
| Corruption detection on replay | Implemented | `aegis/core/crypto_audit.py` (`_load_from_wal`); `tests/test_reliability.py::test_wal_corruption_recovery_partial_chain` | Replay stops at the first malformed line and marks `wal_corrupt`. `_require_intact_ledger` then refuses every governed endpoint with `503` before forwarding or committing, so no node is appended above an unreadable prefix; `/health` and `/metrics` stay reachable so the fault is visible. | Project |
...
| OpenTelemetry tracing | Configuration-dependent | `aegis/core/observability.py` (`setup_otel`, `record_span`); started from `aegis/proxy/app.py` | Requires the `otel` extra; without it `setup_otel` is a no-op and every span is `None`. **Two spans are wired**, `aegis.waf.check` and `aegis.forward` — the evidence commit path carries none. **No W3C trace-context propagation:** no propagator is registered and no `traceparent` is extracted from an incoming request or injected into an upstream one, so a caller's trace does not continue through the gateway. A documented, tested span model across the evidence lifecycle is [Roadmap](../../ROADMAP.md). | You |
| W3C trace-context facade | Present but unwired | `aegis/telemetry/otel.py` | Pure standard library, and **not OpenTelemetry** — it imports nothing from the OTel SDK. It implements `traceparent`/`tracestate` parsing and injection with an attribute allowlist, and it is tested, but **nothing in the request path uses it**. It is listed here so that it is not mistaken for the row above. Wiring it — and with it W3C trace-context propagation through the gateway — is [Roadmap](../../ROADMAP.md) and is deliberately not done here. | You |
