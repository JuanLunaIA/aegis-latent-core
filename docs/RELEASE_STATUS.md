...
**Publication state of `5.0.1` (the checked-out source baseline and most recent published release):** **published on every surface except PyPI `aegis-latent-core`.** Read back 2026-09-24: Sigstore-signed annotated tag (`gitsign verify-tag` passed against the exact workflow identity), GitHub Release with 31 assets (full `SHA256SUMS` sweep: 15 of 15 verified), PyPI `aegis-latent-sdk` `5.0.1`, npm `aegis-latent-sdk` `5.0.1`, and both OCI images, whose signatures pass `cosign verify` and whose build-provenance attestations verify against the exact publishing workflow identity. **The gateway distribution `aegis-latent-core` was not published at `5.0.1`**; its latest on PyPI remains `4.1.2` (see §1.0). `gh attestation verify` itself was not run — the same Sigstore bundles were verified with `cosign`; see §1.0a.
...
**Read this section as five separate things.** The source baseline is `5.0.1`, and it is the most recent published release: **published on every surface except PyPI `aegis-latent-core`** as of the 2026-09-24 readback (§1.0a). The previous release, `5.0.0`, reached the same surfaces as of the 2026-09-16 readback (§1.0). `4.1.2` remains the most recent version published on *every* surface, read back 2026-09-04. The preceding `4.1.1` reached every surface except npm and is superseded. A `v4.1.0` tag and GitHub Release also exist, but neither came from the release pipeline and the release carries no assets. The `4.0.2` rows were last read back on 2026-09-02 and **describe `4.0.2` only**.
...
### 1.0a `5.0.1` — published except the gateway on PyPI, read back 2026-09-24

Released from `main` at `46db6c0c65582bbb46137fd14182a976274b34e8` by one dispatch of `create_release_tag.yml` (run `36016211316`), which dispatched `release.yml` (run `36016251878`), `publish_oci.yml` (run `36016262373`), `publish_pypi.yml` (run `36016254775`) and `publish_npm.yml` (run `36016258024`). Every value below is readback, not workflow status.

| Surface | State | Observed value |
| --- | --- | --- |
| Source baseline | Confirmed | `5.0.1`, fourteen synchronized anchors, contract `READY` on `main` before the tag was cut |
| GitHub tag `v5.0.1` | **Confirmed, signature verified** | Annotated tag object `b590a7f40df939b9efa361f3960bfa8b532296c9` targeting commit `46db6c0c65582bbb46137fd14182a976274b34e8`, an ancestor of `main`. `gitsign verify-tag` (v0.17.1) with `--certificate-identity https://github.com/JuanLunaIA/aegis-latent-core/.github/workflows/create_release_tag.yml@refs/heads/main` and `--certificate-oidc-issuer https://token.actions.githubusercontent.com`: good signature, Rekor entry validated (log index `2940160795`), certificate claims validated |
| GitHub Release `v5.0.1` | **Confirmed** | Created 2026-09-24T14:54:40Z, published 2026-09-24T15:00:29Z, non-draft, non-prerelease, immutable, **31 assets** all in state `uploaded`, target `main` |
| `SHA256SUMS` sweep | **Confirmed** | `SHA256SUMS` lists 15 files; all 15 downloaded and re-hashed: 15 verified, 0 mismatched, 0 listed-but-absent |
| Release-asset attestations | **Confirmed, verified** | Each of the 15 files has a SLSA v1 build-provenance attestation from `release.yml`. All 15 verified (`cosign verify-blob-attestation --new-bundle-format`) against `--certificate-identity https://github.com/JuanLunaIA/aegis-latent-core/.github/workflows/release.yml@refs/heads/main`; every one names commit `46db6c0` and run `36016251878`. Each file also carries GitHub's own immutable-release attestation, signed by GitHub's internal Sigstore CA (`dotcom.releases.github.com`); that one was **not** verified here, because it needs GitHub's trust root rather than the public-good one |
| PyPI (`aegis-latent-core`) | **Not published at 5.0.1** | Registry JSON → `info.version` `4.1.2`; releases are `4.1.2`. Same cause and position as §1.0: no workflow publishes the gateway distribution |
| PyPI (`aegis-latent-sdk`) | **Confirmed published at 5.0.1** | Registry JSON → `info.version` `5.0.1`; releases `4.0.0`, `4.1.1`, `4.1.2`, `5.0.0`, `5.0.1`. Wheel `sha256:7d5b0c9036f6c444…` and sdist `sha256:9cae15cbb9a0d345…`, uploaded 2026-09-24T14:56:29Z/30Z, **byte-identical** to the release assets of the same name |
| npm (`aegis-latent-sdk`) | **Confirmed published at 5.0.1** | Registry JSON → `latest` `5.0.1`; `dist.integrity` `sha512-94X5kw5P…` equals the SHA-512 of the release asset `aegis-latent-sdk-5.0.1.tgz` (**byte-identical**); `dist.attestations.provenance` is SLSA v1 |
| OCI image (gateway) `5.0.1` | **Confirmed** | `ghcr.io/juanlunaia/aegis-latent-core:5.0.1` → `sha256:2d22023ef7f70cc3272950eb98684deb55dd07c2dfcaca9c77ac7e83dd090f3e`, an OCI image index |
| OCI image (dashboard) `5.0.1` | **Confirmed** | `ghcr.io/juanlunaia/aegis-latent-core-dashboard:5.0.1` → `sha256:a05b41c5df4604556a7b148d211a6efd3a5f870ffe31681d7dde2daa2b3722a3`, same index shape |
| Image signatures | **Confirmed, verified** | `cosign verify` (v2.6.5) of each index **digest** with `--certificate-identity https://github.com/JuanLunaIA/aegis-latent-core/.github/workflows/publish_oci.yml@refs/heads/main` and `--certificate-oidc-issuer https://token.actions.githubusercontent.com`: exit 0 for both, transparency-log inclusion and certificate chain verified. The gateway signing certificate names commit `46db6c0` and run `36016262373`. One `.sig` object per image |
| Image build attestations | **Confirmed, verified** | One SLSA v1 provenance attestation per image from the GitHub attestations API, verified with `cosign verify-blob-attestation --new-bundle-format` against the raw index manifest (whose SHA-256 is the image digest) and the `publish_oci.yml` identity; both name commit `46db6c0` and run `36016262373` |
| `gh attestation verify` | `NOT_EXECUTED` | `gh` is not installed on the readback host. The public-good attestation bundles it would check were verified with `cosign` above; GitHub's own release attestations were not |

**How this was verified, and its limits.** The full transcript is [`evidence/v5_0_1_release_readback_2026-09-24.md`](../evidence/v5_0_1_release_readback_2026-09-24.md). `scripts/verify_release_readback.py --tag v5.0.1 --verify-assets` produced the Release, sweep, registry and digest rows. Its own `cosign` row accepts any identity (`--certificate-identity-regexp .*`), so the signature and attestation rows above come from separate runs that pinned the exact workflow identity and OIDC issuer. `cosign` and `gitsign` were built from source through the Go module proxy, which checks each module against the Go checksum database, because this host could not download their release binaries. Verification shows who built and signed each artifact, from which commit, in which workflow run. It does not show that the artifacts are free of defects, that any deployment is configured correctly, or that any external party has accepted them.

**Earlier readback, kept as history:** on 2026-09-21, before the release, the same surfaces returned GitHub Release HTTP 404, OCI tags HTTP 404, and registries at `5.0.0`/`4.1.2`. That was correct for its date and is superseded by the table above.
