...
A fourth file appears **only if you enable cryptographic shredding**: `<wal_path>.shredder.db`, or wherever `AEGIS_SHREDDER_VAULT_PATH` points. It is the opposite of the checkpoint in every operational respect. Deleting it is not safe — it holds the per-subject AES-256-GCM keys, and without them every sealed payload in the WAL is permanently unreadable. It **is** a backup item, and the only one in this system whose backup policy is in tension with what it is for: restoring a pre-erasure vault restores the ability to decrypt everything erased since. Back it up under the same custody as the signing key, and read [Backup and Restore §1](operations/BACKUP_RESTORE.md) before turning sealing on.

### Deciding on cryptographic shredding

`AEGIS_ENABLE_CRYPTOGRAPHIC_SHREDDING` defaults to `false`. Sealing changes what the MMR commits to for each leaf it applies to — the leaf becomes `SHA-256(0x00 || nonce || ciphertext)` instead of the payload digest — but that is a **per-commit decision, so you can switch it on for a chain that already holds records**: commits from that point forward are sealed, earlier ones are untouched, and the mixed chain still verifies. (This guide previously said it could not be switched on for an existing chain, by analogy to `AEGIS_MMR_HASH_SCHEME`; corrected 2026-09-16 — the hash-scheme constraint is unchanged.) The limit to plan around is the reverse: nothing seals records already committed, so enabling the flag does not make existing history shreddable. Budget for it: the WAL then stores ciphertext rather than digests alone, so its growth tracks payload volume rather than staying roughly constant per record. What you get is `crypto_shred(subject_id)`, which destroys one subject's key, is idempotent, and leaves the root, the peaks and every previously issued inclusion proof bit-for-bit unchanged. What you do not get is media sanitisation, or the removal of the request and response digests the node still carries. On a ledger without sealing enabled, `crypto_shred()` raises rather than returning a false success, so a retention job cannot mistake a no-op for an erasure. `CLM-068` governs what may be said about it; no regulatory conclusion follows.
