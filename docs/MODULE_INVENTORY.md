...
| `AUD-39` | P3 | @JuanLunaIA | raise the enforced floor in steps, each step adding tests for the modules furthest below it, largest-gap first: `aegis/storage/s3_worm.py` (80%), `aegis/storage/segment_manifest.py` (76%), `aegis/proxy/forwarder.py` (76%), `aegis/proxy/egress_guard.py` (75%), `aegis/proxy/rate_limiter.py` (77%), `aegis/consensus/` and telemetry modules around 88-92%. Do not raise the floor above the measured value in the same commit that measures it. |

## Files

| Path | Kind | Purpose | Status | Tests | Owner |
|---|---|---|---|---|---|
| `aegis/__init__.py` | package | Aegis Latent Core — forensic evidence gateway for LLM inference pipelines. | reachable | `tests/compat/test_public_api_compat.py`, `tests/test_config_surface_inert_fields.py`, `tests/test_embedded_mode.py`, `tests/test_forensic_pdf_report.py` | @JuanLunaIA |
...
| `aegis/core/__init__.py` | package | aegis.core — Mathematical and cryptographic telemetry primitives. | reachable | `tests/compat/test_public_api_compat.py`, `tests/engines/test_modular_engines.py`, `tests/security/test_enforcement_mode_metric.py`, `tests/security/test_stream_admission_metric.py`, … (+18 more)` | @JuanLunaIA |
...
| `aegis/core/crypto_audit.py` | module | aegis.core.crypto_audit — Cryptographic audit ledger with a Merkle-linked evidence chain. | reachable | `tests/connectors/test_parquet_exporter.py`, `tests/ha/test_ha_failover.py`, `tests/ha/test_ha_sequencer_ledger.py`, `tests/security/test_wal_single_writer.py`, … (+49 more)` | @JuanLunaIA |
...
| `aegis/core/forensic.py` | module | aegis.core.forensic — Forensic record builders for the audit ledger. | reachable | `tests/test_forensic.py`, `tests/test_forensic_builders.py`, `tests/test_part11_annotation_binding.py`, `tests/test_pre_admission_rejection.py`, … (+3 more)` | @JuanLunaIA |
...
| `aegis/core/libc.py` | module | aegis.core.libc — the process's C library, found without starting a process. | reachable | — | @JuanLunaIA |
...
| `aegis/core/mmr.py` | module | aegis.core.mmr — Merkle Mountain Ranges (MMR). | reachable | `tests/compat/test_public_api_compat.py`, `tests/crypto/test_capabilities.py`, `tests/redteam/fuzz/fuzz_mmr_proof.py`, `tests/redteam/test_mmr_proof_forgery.py`, … (+21 more)` | @JuanLunaIA |
...
| `aegis/core/seccomp_guard.py` | module | aegis.core.seccomp_guard — Secure Computing (Seccomp-BPF) Enforcement. | reachable | `tests/test_seccomp_enforced_serving.py`, `tests/test_seccomp_extended.py`, `tests/test_seccomp_guard_new.py`, `tests/test_seccomp_new.py` | @JuanLunaIA |
...
| `aegis/core/zk_native.py` | module | Python entry point for the zero-knowledge inclusion proof. | reachable | `tests/test_zk_native_surface.py`, `tests/test_zk_preview_guard.py` | @JuanLunaIA |
...
| `aegis/proxy/streaming.py` | module | Bounded, cancellation-owned SSE proxy sessions. | reachable | `tests/engines/test_modular_engines.py`, `tests/security/test_stream_admission_metric.py`, `tests/test_forwarder_sse_framing.py`, `tests/test_proxy_streaming.py`, … (+4 more)` | @JuanLunaIA |
| `aegis/proxy/terminal_outbox.py` | module | Durable outbox for terminal stream evidence handed off at teardown (REG-D32). | reachable | `tests/test_terminal_outbox.py` | @JuanLunaIA |
...
| `aegis_server/config.py` | module | aegis_server.config — Enterprise configuration via environment variables. | reachable | `tests/test_config_surface_inert_fields.py`, `tests/test_enterprise_body_limits.py`, `tests/test_enterprise_config_new.py`, `tests/test_enterprise_durable_evidence.py`, … (+5 more)` | @JuanLunaIA |
