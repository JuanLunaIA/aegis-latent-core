...
**Release baseline:** checked-out source baseline `v5.0.0` with 14 synchronized anchors, unpublished
...
| AI/ML engineering | Does it replace provider SDKs? | No. The checked-out `v5.0.0` source exposes an OpenAI-compatible surface and native Anthropic `POST /v1/messages`. Python is drop-in through official-client subclasses. TypeScript wrappers remain provider-native and use the official provider packages as peer dependencies; provider models and behavior remain authoritative. |
