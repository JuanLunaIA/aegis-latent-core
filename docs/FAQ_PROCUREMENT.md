...
**Release baseline:** checked-out source baseline `v5.0.0` with 14 synchronized anchors, published 2026-09-16 on every surface except PyPI `aegis-latent-core`
**Source baseline:** `v5.0.0` with 14 synchronized anchors, **published 2026-09-16 on every surface except PyPI `aegis-latent-core`** — source metadata does not establish external lifecycle state, so each surface was read back separately; `cosign verify`, `gh attestation verify` and the `SHA256SUMS` sweep were **not** run for `5.0.0`. The most recent published release is `v4.1.2`, whose tag, GitHub Release, PyPI, npm, OCI digest and signature objects were read back on 2026-09-04 and are recorded in `docs/RELEASE_STATUS.md` §1.1; `cosign verify` and `gh attestation verify` were not run
