# Data Retention and Privacy Boundaries — Aegis Latent Core v5.0.1 Source Baseline
...
**It is off by default.** With `AEGIS_ENABLE_CRYPTOGRAPHIC_SHREDDING` unset — every deployment that has not deliberately chosen otherwise — the gateway commits payload digests directly and nothing can be shredded: `crypto_shred()` refuses rather than reporting a no-op erasure, so a retention job cannot believe it erased something it never could. Setting it to `true` makes the ledger seal each leaf under a per-subject AES-256-GCM key and commit `SHA-256(0x00 || nonce || ciphertext)` in place of the payload digest. That decision is made **per commit, not per chain**, so the flag can be enabled on a chain that already holds records: commits from that point forward are sealed, earlier ones stay as written, and the mixed chain still verifies. (This paragraph previously said it could not be enabled on an existing chain; corrected 2026-09-16 — see `CLM-068`.) The retention consequence is the asymmetry: only records committed **after** the flag is set are shreddable, because nothing retroactively seals earlier ones, so a schedule that assumes historical records became erasable when the flag was flipped will not survive contact with the ledger.
...
- **It does not remove the node's request and response digests** — those are retained after erasure — **but since `CLM-098` (`REG-012`) they are `HMAC-SHA256` under a salt derived from the subject key**, so destroying the key also destroys the ability to confirm a guessed plaintext against them. The ciphertext, the commitment, the node's existence, timestamp, tenant and chain position are all still retained, and a low-entropy payload elsewhere in the chain remains confirmable by enumeration where digests are not subject-keyed (`CLM-062`).
