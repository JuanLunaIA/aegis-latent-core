# Compliance Contribution Map — Aegis Latent Core v4.1.1 Source Baseline
...
| Backpressure | Injected 2 ms `fsync` delay under 10k RPS offered load for 0.25 seconds preserved 2,500 durable records with valid chain | `tools/benchmarks/run_backpressure_stall.py`, `evidence/execution_2026-08-20/backpressure_stall_report.json` | `MEASURED` / injected seam | p99 commit latency was 836.3514210795984 ms. This is not production capacity, a storage SLO, or `dm-delay` equivalence. |
