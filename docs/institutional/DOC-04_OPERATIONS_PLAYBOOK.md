...
| WAL stall evidence | `evidence/execution_2026-08-20/backpressure_stall_report.json:1-45`; current figure: `evidence/execution_2026-09-16/` | In-tree bounded measurement: 2,500 offered requests with a 2 ms injected delay; the 2026-09-16 group-commit run reports p99 51.87 ms. |
...
| `DOC04-CLM-009` | Whole-set restore is not transactional. Each restored file is now replaced atomically (`_atomic_copy`: temp file in the target directory, flush + fsync, `os.replace`, directory fsync) and that is pinned by a fault-injection test, but an interruption *between* files still leaves a mix of restored and unrestored segments, so the restored file set as a whole is not covered by one transaction. Crash-consistency of the live backup copy is likewise per-file, not transactional. | `ROADMAP` | `aegis/core/wal_backup.py:128-171,245-264`; `tests/test_wal_backup_atomic_restore.py` | The pre-fix code called `shutil.copy2` straight onto the live WAL; the fix replaced it with `_atomic_copy` and the test injects an interrupted copy to prove the live bytes survive. No temporary-file-and-rename transaction spans the whole restored set. | A reviewed implementation and power-loss/concurrent-writer fault tests prove a declared snapshot and atomic replacement boundary. | Evidence custodian |
...
| `DOC04-CLM-011` | The proxy attaches `/metrics` only when `prometheus-client` is importable, which is an optional extra (`aegis[metrics]`); without it every metric is a no-op stub and no endpoint is registered, so metric-based production monitoring depends on an environment-specific install and scrape integration. | `CONFIGURATION-DEPENDENT` | `aegis/proxy/app.py` (`create_app`, `if observability.prometheus_available()`); `aegis/core/observability.py`; `pyproject.toml:83` | No external sidecar or custom application wrapper is assumed. | The deployed artifact exposes and is successfully scraped for all referenced metric series under a reviewed integration. | Observability owner |
...
**Failure branches:** missing/unreadable manifest or WAL, pre-copy integrity failure, copy failure, or post-copy integrity failure blocks traffic. Each restored file is replaced atomically (temp file + fsync + `os.replace` + directory fsync, `WALBackupManager._atomic_copy`), so an interrupted copy cannot tear the live WAL; an interruption *between* files can still leave the target holding a mix of restored and unrestored segments, because no single transaction spans the restored set. Preserve that target, do not append, and restore again to a fresh path from the last verified backup. **Rollback:** select the pre-restore backup only after it independently verifies. **Escalation:** unresolved predecessor, key, or custody mismatch requires evidence and security review; legal-hold or retention impact also requires privacy/legal review.
...
| Restore is atomic per file but not across the restored set | `ROADMAP` | Concurrent writes or an interruption between files can still produce a target that mixes restored and unrestored segments. | Evidence custodian |
