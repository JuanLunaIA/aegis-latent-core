...
| `UC-004` | Streaming is buffered under a configured response limit. | The handler buffers the complete upstream stream but implements no accumulated streaming-response byte ceiling. | `IMPLEMENTED` buffering; response-size resilience is `ROADMAP`. |
