...
| `UC-004` | Streaming is buffered under a configured response limit. | Resolved on 2026-08-21: the handler now enforces per-response byte and total-duration bounds before evidence commit and closes an aborted upstream iterator. Aggregate concurrent memory remains a deployment concern. | `IMPLEMENTED` for one admitted stream; concurrent admission budgeting remains `CONFIGURATION-DEPENDENT`. |
