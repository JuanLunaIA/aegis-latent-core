...
**Source boundary:** checked-out source metadata is synchronized at `v4.3.0`; source metadata does not prove external tag, release, registry, OCI, deployment, compliance, or acceptance state
...
`[LEGAL-REVIEW-REQUIRED]` `aegis/core/crypto_shredder.py` implements envelope encryption with destructible per-subject keys (`CLM-068`; mechanism described in `DOC-02 §6.2`). Since `4.3.0` `CryptographicAuditLedger` can use it, but **only when `AEGIS_ENABLE_CRYPTOGRAPHIC_SHREDDING=true`, which is not the default and cannot be turned on for a chain that already holds records** — sealing changes what the MMR commits to, so it is a decision made when a chain is created. Every statement above about the governed evidence path therefore stands unchanged for a deployment that has not made that decision, which is every deployment that has not deliberately started a new chain for it. This subsection exists so that a reviewer meeting the module does not read into it a compliance conclusion it cannot carry, and the availability of the option changes nothing about which conclusions are available.
