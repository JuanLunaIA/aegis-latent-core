...
One further open question the wiring adds rather than answers: a shredded node **retains its request and response digests**, so a party holding a candidate plaintext can confirm it against the node after erasure. Key destruction removes readability, not confirmability, and the re-identification analysis in the second row above has to account for that.
