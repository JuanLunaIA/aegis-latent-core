...
**Source boundary:** checked-out source metadata is synchronized at `v5.0.1`; source metadata does not prove external tag, release, registry, OCI, deployment, compliance, or acceptance state
...
`[LEGAL-REVIEW-REQUIRED]` `aegis/core/crypto_shredder.py` implements envelope encryption with destructible per-subject keys (`CLM-068`; mechanism described in `DOC-02 §6.2`). Since `4.3.0` `CryptographicAuditLedger` can use it, but **only when `AEGIS_ENABLE_CRYPTOGRAPHIC_SHREDDING=true`, which is not the default**. Sealing is decided per commit rather than per chain, so the flag can be turned on for a chain that already holds records: subsequent commits are sealed, earlier ones stay exactly as written, and the mixed chain still verifies (`CLM-068`; this paragraph previously said the flag could not be enabled on an existing chain, corrected 2026-09-16). Every statement above about the governed evidence path therefore stands unchanged for a deployment that has not set the flag, which is every deployment that has not deliberately opted in. This subsection exists so that a reviewer meeting the module does not read into it a compliance conclusion it cannot carry, and the availability of the option changes nothing about which conclusions are available.
...
One further open question the wiring adds rather than answers: a shredded node retains its request and response digests, and whether a party holding a candidate plaintext can confirm it against the node after erasure depends on the scheme — confirmable where the digests are plain `SHA-256` (a legacy `v1`-sealed node, or any node written with shredding off), not confirmable where they are `HMAC-SHA256` keyed to the subject (`v2`, `CLM-098`), because key destruction takes the derived salt with it. Key destruction removes readability in every case; it removes confirmability only under `v2`, and the re-identification analysis in the second row above has to account for both.

**Approved and blocked wording.** Approved: *the repository contains a mechanism that makes a sealed payload's plaintext unrecoverable to a ciphertext holder while leaving the evidence tree invariant; it is off by default and is enabled per chain, not per request.* Blocked: "GDPR right to erasure satisfied", "Art. 17 compliant", "data destroyed", "sanitised", "forgotten", or any phrasing implying that deploying Aegis discharges an erasure obligation. The controlling boundary is unchanged and is repeated deliberately: **no configuration of this gateway resolves an erasure obligation once an in-scope record has been committed and anchored.**
