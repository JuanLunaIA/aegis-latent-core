...
|---|---|---|---|---|
| Python MMR (v1, the default and what the ledger writes) | SHA-256 leaves; SHA-256 over concatenated lowercase hexadecimal child hashes; root is SHA-256 over height-descending peak hex strings | `aegis/core/mmr.py` | `tests/test_mmr.py:16-46`; `tests/test_mmr_properties.py:40-175` | No domain prefix or binary hash concatenation in this scheme; see the v2 row and §on domain separation. |
| Python MMR (v2, available but not wired into the ledger) | `SHA-256(0x00 \|\| payload)` leaves; `SHA-256(0x01 \|\| left32 \|\| right32)` nodes; root `SHA-256(0x02 \|\| peaks)` over raw 32-byte digests | `aegis/core/mmr.py` (`v2_leaf_hash`, `v2_node_hash`, `v2_bagged_root`) | `tests/test_mmr_domain_separation.py` | Domain-prefixed and binary. `CryptographicAuditLedger` does not use it, so no recorded root changes. `CLM-064`. |
...
The empty root is 64 ASCII zero characters, not `SHA-256` of an empty byte string. These equations describe **v1**, which is the default and the scheme the ledger writes, and they match `aegis/core/mmr.py` and `aegis_rust_v2/src/mmr.rs:18-25,60-115`. v1 carries no leaf/node domain-separation prefix, which is the RFC 6962 §2.1 type-confusion weakness that `aegis-mmr-inclusion-v2` exists to close; v2 is implemented in `aegis/core/mmr.py` and in both SDK verifiers but is not wired into the ledger, so every root recorded by a deployed chain is still a v1 root. See `CLM-064` and [MMR Proof v1](../api/MMR_PROOF_V1.md). The domain-separated Merkle aggregation recorded in `evidence/execution_2026-08-20/manifest.json:1` describes the execution-manifest builder and must not be substituted for the MMR formula.
