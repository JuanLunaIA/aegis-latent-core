...
| MMR | In-process append, SHA-256 root construction, inclusion checks, prefix-peak reconstruction, Python/Rust root parity tests, and a domain-separated `aegis-mmr-inclusion-v2` scheme wired into the ledger — `mmr_hash_scheme` (default `auto`) starts every *new* chain on v2 and reopens an existing chain under whichever scheme its WAL already recorded | A standardized MMR wire format, an in-place scheme transition for an existing v1 chain (none is implemented; the root cannot be recomputed under a new construction without rewriting the history it already committed to), independent consistency-proof verification, distributed global ordering |
...
|---|---|---|---|---|
| Python MMR (v1, what a chain started before `4.3.0` writes) | SHA-256 leaves; SHA-256 over concatenated lowercase hexadecimal child hashes; root is SHA-256 over height-descending peak hex strings | `aegis/core/mmr.py` | `tests/test_mmr.py:16-46`; `tests/test_mmr_properties.py:40-175` | No domain prefix or binary hash concatenation in this scheme; see the v2 row and §on domain separation. |
| Python MMR (v2, what a **new** chain writes) | `SHA-256(0x00 \|\| payload)` leaves; `SHA-256(0x01 \|\| left32 \|\| right32)` nodes; root `SHA-256(0x02 \|\| peaks)` over raw 32-byte digests | `aegis/core/mmr.py` (`v2_leaf_hash`, `v2_node_hash`, `v2_bagged_root`) | `tests/test_mmr_domain_separation.py` | Domain-prefixed and binary. `AEGIS_MMR_HASH_SCHEME` defaults to `auto`: a new chain starts here, an existing one reopens under the scheme its WAL recorded, and **no root already recorded changes**. `CLM-064`. |
...
| Envelope encryption for erasure (wired, **off by default**) | AES-256-GCM from `cryptography`, 96-bit `os.urandom` nonce per seal, subject id bound as associated data; commitment is `SHA-256(0x00 \|\| nonce \|\| ciphertext)` | `aegis/core/crypto_shredder.py` (`CryptoShredder.seal`, `SealedPayload.commitment`); `aegis/core/crypto_audit.py` (`_seal_leaf`) | `tests/test_crypto_shredder.py`; `tests/test_crypto_shredder_integration.py` | No hand-rolled primitive: no bespoke commitment scheme, curve arithmetic, or custom KDF. With `AEGIS_ENABLE_CRYPTOGRAPHIC_SHREDDING` unset the ledger commits payload digests directly, exactly as before; set, the MMR leaf becomes the commitment above. The choice is made per commit rather than per chain, so enabling it on an existing chain seals subsequent leaves and leaves earlier ones untouched; `AuditNode.shredding_version` distinguishes them. §6.2; `CLM-068`. |
...
The empty root is 64 ASCII zero characters, not `SHA-256` of an empty byte string. These equations describe **v1**, and they match `aegis/core/mmr.py` and `aegis_rust_v2/src/mmr.rs:18-25,60-115`. v1 carries no leaf/node domain-separation prefix, which is the RFC 6962 §2.1 type-confusion weakness that `aegis-mmr-inclusion-v2` exists to close. Which of the two a ledger writes is a property of the **chain**, not of the release: `AEGIS_MMR_HASH_SCHEME` defaults to `auto`, so a chain created from `4.3.0` onward records v2 roots and a chain already in the field goes on recording v1 roots under the scheme its WAL declares. No root that has been recorded changes, in either direction — a root cannot be recomputed under a different construction without rewriting the history it commits to, which is why there is no in-place upgrade. See `CLM-064` and [MMR Proof v1](../api/MMR_PROOF_V1.md). The domain-separated Merkle aggregation recorded in `evidence/execution_2026-08-20/manifest.json:1` describes the execution-manifest builder and must not be substituted for the MMR formula.
...
An MMR over `n` leaves decomposes into a forest of perfect binary trees whose heights are the set bits of `n`. The number of peaks is therefore exactly the population count of the leaf count, and the implementation enforces this as a verification predicate rather than treating it as an informal property (`MerkleMountainRange.verify_portable_inclusion_hash` in `aegis/core/mmr.py`):

\[
|\mathrm{peaks}| = \operatorname{popcount}(n).
\]

Bagging orders peaks by **descending height**, concatenates their hexadecimal text, and hashes once (`MerkleMountainRange.get_root_hash` in `aegis/core/mmr.py`; Rust parity at `aegis_rust_v2/src/mmr.rs:111-114`):
...
The portable proof is a self-contained envelope that a verifier can check without any local MMR state. Its exact field set is fixed by `MMRInclusionProofV1` (`aegis/core/mmr.py`) and pinned by the cross-language vector corpus `sdk/shared/mmr-inclusion-v1.json`:
...
Verification is strict and ordered (`MerkleMountainRange.verify_portable_inclusion_hash` in `aegis/core/mmr.py`): the leaf digest must be SHA-256 hexadecimal; `version` and `algorithm` must match their literals; index bounds must hold; the supplied `trusted_root` must itself be SHA-256 hexadecimal **and** equal `proof.root`; the peak count must equal `popcount(leaf_count)`; and `peak_index` must address a real peak. Only then is the path folded and the peak list bagged.
...
`MerkleMountainRange.add_leaf` hashes raw leaf bytes, merges equal-height rightmost peaks, and stores parent/child relations in `MerkleMountainRange.add_leaf` and `add_leaf_hash`. `get_root_hash` sorts peaks by height descending and hashes their concatenated text in `MerkleMountainRange.get_root_hash`. The Rust implementation mirrors those transformations at `aegis_rust_v2/src/mmr.rs:60-115`.

Inclusion verification recomputes the sibling path, checks that the result equals one of the object’s current peaks, then recomputes the root from **the verifier object’s complete current peak set** in `MerkleMountainRange.verify_inclusion`. Therefore a caller must verify against an MMR instance representing the claimed state. The proof list alone is not a self-contained commitment to all other peaks.
...
**How it reaches a chain.** `CryptographicAuditLedger` seals a leaf only when `AEGIS_ENABLE_CRYPTOGRAPHIC_SHREDDING=true`; unset — the default — it commits payload digests directly and nothing above applies to it. Sealing is decided per commit, not per chain, so it **can** be switched on for a chain that already holds records: `_seal_leaf` branches on whether the shredder is configured at the moment each leaf is built, so subsequent commits are sealed, earlier ones stay as written, and the mixed chain still verifies — `AuditNode.shredding_version` is what tells the two leaf kinds apart. (This paragraph previously said the opposite, and cited the hash-scheme rule in §5 by analogy; that analogy was wrong and is corrected 2026-09-16. The hash-scheme rule itself is unchanged.) What has no migration path is the reverse: nothing retroactively seals leaves already committed as plain digests. Three consequences an operator owns. The WAL carries ciphertext rather than digests alone, so its size tracks payload volume. The key vault — `<wal_path>.shredder.db` unless `AEGIS_SHREDDER_VAULT_PATH` says otherwise — becomes the only mutable component in an append-only design and therefore a new single point of failure: lose it and every subject's plaintext is gone at once, restore it from a backup and every erasure performed through it is undone, which puts its backup policy in direct tension with the erasure it exists to perform. And a shredded node still carries its request and response digests, where what a guess can do with them depends on the scheme: plain `SHA-256` on a legacy `v1`-sealed node, so erasure there removed readability but not the ability to confirm a guess; `HMAC-SHA256` under a salt derived from the subject key on a `v2` node, so erasure removes both (`CLM-098`).

**No regulatory conclusion follows** from any of it: this volume describes a mechanism, it is not legal advice, and the erasure-obligation analysis stays where it belongs, in `DOC-05 §5.8` (`CLM-068`).
...
|---|---|---|---|---|---|---|---|
| DOC02-CRY-001 | The Python MMR implements the SHA-256 and hex-text construction defined in Section 5. | `IMPLEMENTED` | `aegis/core/mmr.py` (`add_leaf`, `add_leaf_hash`, `get_root_hash`); `tests/test_mmr.py:16-46`; `tests/test_mmr_properties.py:40-175` | Input bytes and ordered append sequence are preserved. | A cited test fails, or code changes leaf, merge, peak order, encoding, or empty-root behavior. | One in-process MMR state. | Cryptography Review Owner |
...
| DOC02-CRY-003 | MMR inclusion verification requires the verifier object’s current peak set and is not a proof-only verifier. | `IMPLEMENTED` | `aegis/core/mmr.py` (`MerkleMountainRange.verify_inclusion`) | The object reflects the claimed MMR state. | A reviewed code change validates all peaks solely from a self-contained proof. | Current Python object state. | Cryptography Review Owner |
| DOC02-CRY-004 | Returned consistency data reconstruct old peaks but do not constitute a fail-closed independently verified consistency-proof protocol. | `IMPLEMENTED` | `aegis/core/mmr.py` (`MerkleMountainRange.get_consistency_proof`); tests at `tests/test_mmr_branch.py:58-132` | Internal node history is available. | A production verifier rejects old-root mismatch and validates append-only extension from self-contained proof data. | Prefix reconstruction in one MMR instance. | Cryptography Review Owner |
| DOC02-CRY-005 | SHA-256 is the active digest for the audited MMR and forensic commitments. | `IMPLEMENTED` | `aegis/core/mmr.py` (v1 leaf, merge and peak-bagging construction); `aegis/core/forensic.py:27-29`; `aegis/core/forensic_pdf_report.py:109-119`; `aegis/core/iso27037_evidence.py:305-331` | Exact subsystem serialization is retained. | Any cited path changes algorithms or representations without claim update. | Listed paths only. | Security Architecture Owner |
