...
| MMR | In-process append, SHA-256 root construction, inclusion checks, prefix-peak reconstruction, Python/Rust root parity tests, and a domain-separated `aegis-mmr-inclusion-v2` scheme available to callers and verifiers | A standardized MMR wire format, domain-separated hashing **in the ledger's own accumulator** (v2 is implemented but not wired in; v1 remains the default), independent consistency-proof verification, distributed global ordering |
