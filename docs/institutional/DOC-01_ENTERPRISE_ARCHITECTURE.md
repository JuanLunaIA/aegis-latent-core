...
| Streaming is buffered under a configured response bound. | Streaming is fully buffered before commit and client emission, but no streaming-response byte bound is implemented in the handler. The configured `max_request_body_bytes` limits inbound request bodies, and `MAX_PAYLOAD_BYTES` rejects oversized request bytes in the ledger; neither bounds accumulated upstream response bytes. | `aegis/proxy/app.py:1089-1137`; `aegis/proxy/app.py:85-119`; `aegis/core/crypto_audit.py:63,358-359`; `tests/test_p0_release_gates.py::test_request_body_limit_rejects_declared_oversize`. |
...
| `UPSTREAM PENDING` | Streaming chunks arrive | `STREAM BUFFERING` | No; nothing is yielded to client | `aegis/proxy/app.py:1089-1106` | Provider termination semantics: `tests/test_provider_contracts.py::TestOpenAIProvider::test_explicit_done_in_stream_yields_done` and `test_stream_ending_without_message_stop_emits_done`; no direct byte-bound test exists. |
...
| DOC01-STRM-002 | Core streaming buffering has a configured response-size bound. | `ROADMAP` | Absence in `aegis/proxy/app.py:1089-1137`; inbound-only bound at `aegis/proxy/app.py:85-119`; request-only hard cap at `aegis/core/crypto_audit.py:358-359` | A bound and deterministic failure/evidence policy are added | A stream over the declared bound remains fully accumulated or emitted | Not implemented today | Reliability owner |
...
| DOC01-RISK-002 | Streaming response buffering is unbounded. | Memory exhaustion, long-lived requests, and loss of availability before evidence commit. | Ingress request bound does not mitigate response growth. | Add response byte/time bounds and tests. | Reliability owner |
