...
| Replay reconstructs the complete cryptographic accumulator. | Correct for `CryptographicAuditLedger` as of 2026-09-03: `_load_from_wal` replays every persisted leaf hash through `add_leaf_hash`, so a reopened ledger's root equals an uninterrupted ledger's root. This supersedes the previous correction on this row, which stated that replay appended nodes without replaying leaves and that roots restarted from a fresh accumulator — that described the code before the replay path was covered by tests. It remains **false** for the separate enterprise accumulator in `aegis_server/main.py`, which is initialised empty per process (see §7). Cross-*replica* continuity does not exist for either. | `aegis/core/crypto_audit.py` (`_load_from_wal` → `self._mmr.add_leaf_hash`); `tests/test_mmr_restart.py` (root equality at every tree shape, proof verification after restart, no drift across repeated restarts). Superseded basis: `_load_from_wal` appends nodes but does not call `_mmr.add_leaf`. |
| A multi-worker PostgreSQL enterprise deployment has an atomic chain append. | **Corrected 2026-09-21.** The guard exists now, and the claim is still blocked for a different reason: `write_node_atomic` appends only against the expected tip and refuses a stale one (`aegis_server/storage/base.py:245`; SQLite `BEGIN IMMEDIATE` `:378`; PostgreSQL `:296`; DynamoDB `:237`; introduced in #156), and it is the enterprise server's append path (`aegis_server/main.py:593`). What the claim cannot say is anything about *the deployment*: the guard is per chain in one backend, it does not coordinate replicas that each hold their own chain, and the gateway's own ledger never calls it — that ledger is single-writer by contract (`UC-005`, `AD-16`). | `aegis_server/storage/base.py:245`; `aegis_server/storage/{sqlite,postgres,dynamodb}_provider.py`; `aegis_server/main.py:593` |
...
| WAL corruption is always fail-closed. | Correct at ingress as of 2026-09-03: `_require_intact_ledger` runs at the top of all three governed endpoints and returns `503` before any forwarding or commit while `_fault_state` is not `healthy`. This supersedes the previous correction on this row, which stated that commits remained permitted after replay marked `wal_corrupt`. The guard is triggered by the ledger's own fault state, so it does not detect corruption that replay parses successfully, corruption introduced after startup, or tampering outside the WAL. | `aegis/proxy/app.py` (`_require_intact_ledger`); `tests/test_app_wal_corrupt.py` (refusal on all three endpoints, before the upstream call, with no chain growth); claim boundary `CLM-059` in `docs/CLAIMS_MATRIX.md`. Superseded basis: the request path did not check the fault state before committing. |
...
| Enterprise app with shared SQLite | Awaited row commits; a stale-tip append is refused rather than forked (`write_node_atomic`, #156) | Single request's storage call completed | Concurrent fork-free append *across separate chains or replicas* | Database owner |
| Enterprise app with PostgreSQL/DynamoDB | Awaited backend write; the per-chain race is closed by atomic compare-and-append (`write_node_atomic`, #156) | Backend acknowledged individual record under its configuration; one chain cannot fork under concurrent workers | Atomic global append or cross-region total order | Database/cloud owner |
| Dedicated centralized writer | Architectural option only; declined for the ledger by `AD-16` (2026-09-21) | `ROADMAP` | Available global ordering | Architecture review board |
...
| DOC01-EXT-002 | External storage produces a fork-free total chain under concurrent workers. | `IMPLEMENTED` for one chain, not for a global order | Atomic compare-and-append on the provider interface (`aegis_server/storage/base.py:245`; SQLite/PostgreSQL/DynamoDB providers), used by `aegis_server/main.py:593` | Nothing for a single chain; a global order would still need a centralized writer or consensus | Two successful nodes reference the same non-genesis predecessor in a concurrency test | Covered by `tests/storage/test_chain_fork_prevention.py` and the two provider race tests; the gateway ledger does not use the guard (`AD-16`) | Database architecture owner |
...
| DOC01-FALS-007 | Seed a malformed JSONL line, restart, and submit a governed request. | Governed traffic is refused with `503` before forwarding or commit while `_fault_state` is not `healthy`; the chain does not grow. | `tests/test_app_wal_corrupt.py` (all three governed endpoints, refusal before the upstream call, no chain growth). `tests/test_reliability.py::test_wal_corruption_recovery_partial_chain` covers replay truncation, not ingress. | `IMPLEMENTED` since 2026-09-03 — see Table 2 | A new governed record is accepted while the ledger reports `wal_corrupt`. | Incident-response owner |
...
| DOC01-RISK-003 | Corruption that replay parses successfully is not detected. | The ingress guard closed the fail-open path this row previously described (see Table 2), but it is triggered by the ledger's own fault state — so a WAL damaged in a way replay accepts, damaged after startup, or tampered with outside the WAL raises no fault and traffic continues. | `_require_intact_ledger` keys on `_fault_state`; `CLM-059` states the same boundary. | Periodic full-chain verification independent of replay. | Incident-response owner |
...
| DOC01-RISK-007 | **Historically phrased (pre-#156); mitigated per chain 2026-09-21.** External provider chain append was read-then-write without atomic head control. | Concurrent enterprise nodes could form branches even when every row was durable. | Atomic compare-and-append, implemented: `write_node_atomic` refuses a stale tip (`ConcurrentChainMutationError`) and is covered by `tests/storage/test_chain_fork_prevention.py`; residual risk is ordering *across* chains or replicas, where the gateway ledger stays single-writer by decision (`AD-16`). | Atomic compare-and-append or centralized writer — the first is implemented, the second declined (`AD-16`). | Database architecture owner |
