# Security FAQ — Aegis Latent Core v4.1.0 source
...
**Source baseline:** `v4.1.0`; source metadata is not publication, deployment-acceptance, or certification evidence
**Retained evidence baseline:** `v3.1.0`; retained measurements remain historical and are not v4.1.0 measurements
...
## Are the Phase 2/3 streaming, SDK, proof, export, and dashboard modules in the v4.1.0 source?
Yes. They are implemented in the `v4.1.0` source and are covered by the code, tests, CI contracts, and bounded evidence locators in `docs/CLAIMS_MATRIX.md`. The immutable `v3.1.0` tag and its release assets pre-date those modules. The v4.1.0 source version alone does not prove that a signed tag, GitHub Release, packages, OCI images, or attestations exist; each publication boundary requires independent readback.
