...
- [ ] **If evaluating an SDK from a registry, confirm the version.** PyPI and npm both carry `4.1.2` (readback 2026-09-04). The source tree's anchors read `4.3.0`, which is unpublished — so a registry install is `4.1.2` code and will not contain anything documented as newer.
