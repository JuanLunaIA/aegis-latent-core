...
async def _run_forensic_analytics(
    *,
    ...
    # ── 3. Shannon entropy of response text ───────────────────────────
    entropy: float = 0.0
    try:
        if token_trail:
            log_probs_arr = np.array([t["logprob"] for t in token_trail], dtype=np.float64)
            # Convert log-probs (base-e) to probabilities, then compute entropy
            probs = np.exp(np.clip(log_probs_arr, -80.0, 0.0))
            probs = probs / (probs.sum() + 1e-12)  # normalise
    ...
    # ── 5. Build node_data ────────────────────────────────────────────
    # Retrieve the previous node_id for chain linkage.
    prev_hash: str = "0" * 64
    try:
        recent = await storage.list_nodes(limit=1, offset=0)
        if recent:
            prev_hash = recent[-1].get("node_id", "0" * 64)
    except Exception as exc:
        logger.debug("request_id=%s: could not retrieve prev_hash: %s", request_id, exc)

    # The merkle_root is derived from a deterministic hash of content fields.
    # In a full deployment this delegates to aegis.core.mmr.MerkleMountainRange.
    # Here we use a stable surrogate: SHA-256(request_hash || response_hash || ts).
    merkle_root: str = hashlib.sha256(
