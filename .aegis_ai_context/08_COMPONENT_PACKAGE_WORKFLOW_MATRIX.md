...
| `python-sdk` | [`sdk/python/pyproject.toml`](../sdk/python/pyproject.toml) `project.version` | Python distribution `aegis-latent-sdk` | PyPI 4.0.0 is a separate prior public observation; source-target 5.0.0 metadata is not publication evidence (PyPI `aegis-latent-core` remains at 4.1.2). |
