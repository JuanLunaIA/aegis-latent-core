...
This map separates **package identity**, **version synchronization**, **runtime linkage**, **GitHub publication**, **registry observation**, and **provenance**. Historical immutable source baseline `fdace8844568eb788216740b2cb5daf187d99d3b` has 14 anchors at `4.0.0`. Historical published GitHub Release `v4.0.1` is a lightweight tag targeting `6469904380218584ae0b5221334bc9a46500f5ba`. Prior public PyPI/npm `aegis-latent-sdk` packages were observed at `4.0.0`, without attribution to failed workflows. The source release target is `v5.0.1` with 14 synchronized `5.0.1` anchors and is **published 2026-09-24 on every surface except PyPI `aegis-latent-core`**. The previous release, `v5.0.0`, was **published on 2026-09-16 to every surface except PyPI `aegis-latent-core`**: signed tag, GitHub Release with 31 assets, PyPI `aegis-latent-sdk`, npm `aegis-latent-sdk`, and both GHCR images, each read back. `4.1.2` remains the most recent version published on *every* surface, read back on 2026-09-04, and is still what `pip install aegis-latent-core` resolves to. External lifecycle state (tag, GitHub Release, PyPI, npm, OCI, and attestations) must be independently read back and is never encoded by source metadata.

## Fourteen synchronized version anchors

The authoritative loader is `_load_versions` in [`scripts/verify_release_contract.py`](../scripts/verify_release_contract.py). Every row in the checked-out source release target resolves to `5.0.1`; this does not alter the independent historical source-baseline, GitHub Release, prior registry observations, or current external lifecycle state.
...
| `python-sdk` | [`sdk/python/pyproject.toml`](../sdk/python/pyproject.toml) `project.version` | Python distribution `aegis-latent-sdk` | PyPI 4.0.0 is a separate prior public observation; source-target 5.0.1 metadata is not publication evidence (PyPI `aegis-latent-core` remains at 4.1.2). |
...
| `typescript-sdk` | [`sdk/typescript/package.json`](../sdk/typescript/package.json) `version` | npm package `aegis-latent-sdk` | npm carries `5.0.0` as of the 2026-09-16 readback; the version list is `4.0.0`, `4.1.2`, `5.0.0`, since the `4.1.1` publish step failed and was never rerun. Source metadata is never publication evidence. |
...
| [`publish_pypi_gateway.yml`](../.github/workflows/publish_pypi_gateway.yml) | Build the gateway package (`aegis-latent-core`: aegis + aegis_server + integrations) and conditionally publish exact artifacts | Requires a matching signed tag, main ancestry, a separate `pypi-gateway` environment, OIDC, and `AEGIS_TRUSTED_PUBLISHING_ENABLED == 'true'`; added 2026-09-26, never yet dispatched, so no run has been observed to succeed or fail. The most recently published `aegis-latent-core` on PyPI is still `4.1.2` and stays so until this workflow runs and its own PyPI Trusted Publisher (a PyPI-side action outside this repo) is configured. |
