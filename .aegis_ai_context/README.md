...
Keep the evidence classes independent. The **historical immutable source baseline** is Git commit **`fdace8844568eb788216740b2cb5daf187d99d3b`**, where all **14 version anchors read `4.0.0`**. The **historical published GitHub Release** is **v4.0.1**; its lightweight tag targets commit **`6469904380218584ae0b5221334bc9a46500f5ba`**. Prior public **PyPI and npm registry observations** show the `aegis-latent-sdk` packages at **`4.0.0`**; registry existence does not establish which workflow produced them, so no provenance is attributed to failed or other workflow runs. The **current source baseline** is **v5.0.0**, with **14 synchronized anchors at `5.0.0`**, and it was **published 2026-09-16 on every surface except PyPI `aegis-latent-core`**; there is no `4.2.0` at any surface. The **most recent published release** is **v4.1.2**, read back on 2026-09-04: signed annotated tag targeting **`860f14177d94c194e5ae7156017d6fa74264e429`**, GitHub Release with **31 assets**, **PyPI `aegis-latent-core` `4.1.2`**, **PyPI `aegis-latent-sdk` `4.1.2`**, **npm `aegis-latent-sdk` `4.1.2`**, and GHCR gateway/dashboard images carrying cosign signature objects. The superseded **`4.1.1`** reached every surface except npm. `cosign verify` and `gh attestation verify` were not run for `4.1.2`. The **previous published release** is **v4.0.2**: its signed annotated tag and GitHub Release target commit **`a6eb58dcc03f8b638c8f3e35f0300f5443a926ca`**; GHCR manifests, GitHub attestations, and keyless OCI signatures were independently verified on 2026-08-28, while both SDK publish jobs were skipped so neither registry received `4.0.2`. Record `git rev-parse HEAD` and `git status --short`; manifest hashes are neither release identity nor external lifecycle evidence.
