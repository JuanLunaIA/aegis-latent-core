...
- **The same class one layer out, closed and gated (`REG-D56`).** The restructure above covered the artifacts whose sentence fused *target* and *publication*; it did not reach the wider corpus, where 53 files still named `5.0.0` as their checked-out source baseline — invisible to the repository-wide currency gate because its superseded-token was pinned to `4.1.2` (whose comment still said "because that is `5.0.0`"). 116 sentences restructured into the same two-fact form, review stamps untouched (`AUD-37` owns content reviews; noted there); the gate now carries both superseded tokens, frames `registr(?:y|ies)`, exempts the three dated records it would otherwise misread (`IMPLEMENTATION_LOG_5.0.1.md`, `STATE_MANIFEST.md`, `docs/commercial/ARTIFACT_INVENTORY.md`), and flags the `not wired in <version>` warning family (`_STALE_WIRING`); the user-facing "Not wired in 5.0.0" strings (15 warnings, 4 presets, one doc row, one test docstring) now name `5.0.1`, pinned by `test_inert_warnings_name_the_checked_out_release`; `dashboard/package-lock.json`'s linked-SDK copy and the two agent-facing files (`.github/copilot-instructions.md`, `.claude/agents/release-truth-auditor.md`) corrected; the AI-context manifest regenerated (83 files). `tests/test_documentation_currency.py`.
