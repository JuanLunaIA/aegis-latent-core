...
<<<<<<< HEAD
    ...
    (``t["logprob"]``) that arrive when logprobs are deserialized from JSON
    without going through the Pydantic schema layer.
    """
    if not top_logprobs:
        return np.array([1.0])
    ...
    for t in top_logprobs:
        if isinstance(t, dict):
            lp_values.append(float(t.get("logprob", -10.0)))
        else:
...
>>>>>>> c11319c9d5522df98ed3727694afe7eacd82bee0
        ...
        for i, tok in enumerate(content):
            ...
            logits = _logprobs_to_numpy(top_logprobs)
            ...
            if self._monitor._baseline_dist is not None:
                kl_res = self._monitor.compute_kl_divergence(
                    pseudo_logits, self._monitor._baseline_dist
                )
                kl = kl_res.value
                _tok_lbl = tok.get("token", "?") if isinstance(tok, dict) else tok.token
                if kl > self.kl_threshold * 2:
                    tok_alert = self._make_alert(
                        ...
                        kl,
                        self.kl_threshold * 2,
                        f"Token {i} ('{_tok_lbl}'): Divergence from baseline KL={kl:.4f}",
                    )
            ...
            if self._prev_logits is not None:
                ...
                tok_str = tok.get("token", "?") if isinstance(tok, dict) else tok.token
                if not kl_res.saturated and kl > self.kl_threshold:
                    ...
                    tok_alert = self._make_alert(
                        ...
                        kl,
                        self.kl_threshold,
                        f"Token {i} ('{tok_str}'): KL={kl:.4f} > threshold={self.kl_threshold}",
                    )
                    ...
                    tok_alert = self._make_alert(
                        ...
                        js,
                        self.js_threshold,
                        f"Token {i} ('{tok_str}'): KL saturated, JS={js:.4f} > threshold={self.js_threshold}",
                    )
            ...
            if self._baseline_entropy is not None:
                ...
                if drop > self.entropy_alert_drop_bits:
                    ...
                    _tok_label = tok.get("token", "?") if isinstance(tok, dict) else tok.token
                    collapse_alert = self._make_alert(
                        ...
                        "entropy_drop_bits",
                        drop,
                        self.entropy_alert_drop_bits,
                        f"Token {i} ('{_tok_label}'): entropy dropped {drop:.4f} bits below baseline",
                    )
        ...
        mean_e = float(np.mean(finite_entropies)) if finite_entropies else 0.0
        min_e = float(np.min(finite_entropies)) if finite_entropies else 0.0
        max_e = float(np.max(finite_entropies)) if finite_entropies else 0.0
