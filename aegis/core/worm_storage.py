...
class WORMStorageProvider:
    ...
    async def write_entry(self, data: bytes) -> int:
        ...
        entry = WORMEntry(
            index=self._next_index, timestamp=time.time(), data=data, checksum=checksum
        )

        self._storage[self._next_index] = entry
        logger.info(
            "Data written to WORM storage at index %d. Checksum: %s", self._next_index, checksum
        )

        self._next_index += 1
        return self._next_index - 1

    async def read_entry(self, index: int) -> bytes | None:
