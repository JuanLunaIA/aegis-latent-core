...
class SemanticDriftMonitor:
    ...
    def analyze_logits(self, current_logits: np.ndarray) -> DriftResult:
        ...
        if entropy < self.entropy_floor:
            is_anomaly = True
            action = "KILL_SWITCH"
            logger.critical("SEMANTIC COLLAPSE: Entropy dropped below floor (%f). Potential Hijack!", entropy)
        
        # Case B: High Divergence (Model output shifts drastically from baseline)
        elif divergence > self.kl_threshold:
            is_anomaly = True
            action = "ALERT"
            logger.warning("SEMANTIC DRIFT: High KL Divergence (%f). Unusual output pattern.", divergence)
