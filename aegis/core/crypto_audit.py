...
class AuditNode:
    ...
    def __post_init__(self) -> None:
        # Snapshot the node hash immediately after construction.
        # verify_integrity() compares against this snapshot to detect any
        # in-memory mutation of node fields after creation.
        #
        # NOT a dataclass field — dataclasses.asdict() will NOT include it,
        # so serialisation / from_dict() round-trips are unaffected.
        # Nodes reconstructed from storage via from_dict() also call __post_init__,
        # so they receive a fresh snapshot from their loaded field values.
...
class CryptographicAuditLedger:
    ...
    def verify_integrity(self) -> tuple[bool, int | None]:
        ...
        for i, node in enumerate(chain_list):
            # 1. Hash self-consistency: detect in-memory field mutations.
            # We compare the node's current dynamic hash against the snapshot
            # captured in __post_init__.  Any mutation of state_id, entropy,
            # merkle_root, signature, request_hash, response_hash, etc. after
            # construction will change node.node_hash and trigger this check.
