...
class CFIManager:
    ...
    def verify_binary_cfi(self, binary_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if is_cfi_enabled:
                logger.info("CFI Verification: Found valid Control Flow Integrity markers in %s", binary_path)
                return True, "CFI protections verified (LLVM-CFI / Shadow Stack)"
            else:
                logger.critical("CFI Verification FAILURE: Binary %s is missing CFI protections!", binary_path)
