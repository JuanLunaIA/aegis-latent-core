...
class KernelHardener:
    ...
    def check_kernel_lockdown(self) -> LockdownState:
        ...
        try:
            ...
            with open(self.LOCKDOWN_PATH, "r") as f:
                ...
                if "[integrity]" in content:
                    ...
                    return LockdownState(enabled=True, mode="confidentiality", policy="Confidentiality")
                else:
    ...
    def enforce_hardening(self) -> bool:
        ...
        if lockdown.mode == "none" and self.required_mode != "none":
            logger.error("CRITICAL: Kernel Lockdown is DISABLED. Root user can modify kernel memory.")
            return False
        
        if lockdown.mode == "integrity" and self.required_mode == "confidentiality":
            logger.error("CRITICAL: Lockdown mode 'integrity' is insufficient. 'confidentiality' required.")
            return False
