...
class MoERoutingMonitor:
    ...
    def calibrate_from_samples(self, samples: list[np.ndarray], expert_norms: np.ndarray) -> None:
    ...
    def detect_entanglement(
        ...
        if np.any(gate_weights < 0.0) or np.any(gate_weights > 1.0):
            return EntanglementResult(
                True, 0.0, 0.0, 0.0, "INVALID_INPUT", "Gate weights must be in [0, 1]"
            )
        ...
        if sum_gates == 0:
            return EntanglementResult(
                ...
                float("nan"),
                float("nan"),
                "CATASTROPHIC_ROUTING",
                "All gates are zero",
            )

        if expert_norms is not None and expert_norms.shape != gate_weights.shape:
            raise ValueError("gate_weights and expert_norms must have the same shape")
        ...
        absolute_activation_violated = (
            aggregate_activation > self.activation_bound and max_gate < self.gate_threshold
        )
        ...
        return EntanglementResult(
            False, routing_entropy, aggregate_activation, max_gate, "NOMINAL", "Within parameters"
        )
