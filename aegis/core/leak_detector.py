...
class DataLeakDetector:
        ...
        for pattern in self.secret_patterns:
            for match in pattern.finditer(text):
                ...
                if entropy > self.entropy_threshold:
                    leaks.append((start, end, entropy, f"High-entropy pattern match: {pattern.pattern}"))
    ...
    def is_leaking(self, text: str) -> Tuple[bool, str | None]:
        ...
        if leaks:
            ...
            return True, f"Leak detected at {best_leak[0]}:{best_leak[1]} with entropy {best_leak[2]:.4f} ({best_leak[3]})"
