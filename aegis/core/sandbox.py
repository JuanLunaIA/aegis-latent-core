...
class SeccompFilter:
    ...
    def transition_to_phase(self, phase: str, new_syscalls: List[int]):
        ...
        logger.info("Sandbox phase updated. New syscall surface size: %d", len(self.current_allowed))

    def apply(self):
        """Applies the current filter to the process."""
        try:
            logger.info("Applying seccomp-bpf filter for phase %s. Allowed: %s", self._phase, self.current_allowed)
...
def enable_hardened_sandbox(phase: str = "INIT"):
    ...
    init_syscalls = [
        SYS_READ, SYS_WRITE, SYS_CLOSE, SYS_RT_SIGRETURN, 
        SYS_MUNMAP, SYS_BRK, SYS_EPOLL_WAIT, SYS_ACCEPT, 
        SYS_SENDTO, SYS_RECVFROM, SYS_EXIT_GROUP, SYS_FUTEX,
        0, 1, 2 # open, openat, etc.
    ]
